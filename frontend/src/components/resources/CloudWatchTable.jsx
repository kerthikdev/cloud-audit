...
export default function CloudWatchTable({ items }) {
    ...
    const logGroups = pg.paged.filter(r => r.raw_data?.resource_subtype === 'log_group' || r.raw_data?.retention_days !== undefined)
    const alarms = pg.paged.filter(r => r.raw_data?.resource_subtype === 'alarm' || 'alarm_name' in (r.raw_data || {}))

    return (
        <div>
            <ResourceFilter items={items} onFiltered={setFiltered} extraFields={['log_group_name', 'alarm_name']} />
